...
def setup_logger(name, save_dir, distributed_rank, filename="log.txt", steps=False, verbose=False):
    ...
    try:
        ...
        if distributed_rank > 0:
            return logger
        if save_dir:
            logger.add(os.path.join(save_dir, filename),  format='{time:YYYY-MM-DD HH:mm:ss.SSS} | <level>{level: <8}</level> | {name}{function}{line}</cyan> - <level>{message}</level>', colorize=False, level=level)
